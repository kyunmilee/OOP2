...
int main() {
    ...
    cout << "INVOKING TOSTRING FUNCTION ON BASE-CLASS AND "
        << "\nDEIVED CLASS OBJECTS WITH DYNAMIC BINDING";

    const CommissionEmployee* commissionEmployeePtr{&commissionEmployee};
    cout << "\n\nCALLING VIRTUAL STRING TOSTRING WITH BASE-CLASS POINTER "
        << "TOSTRING FUNCTION:\n"
        << commissionEmployeePtr->toString();
    
    
    const BasePlusCommissionEmployee* basePlusCommissionEmployeePtr{&basePlusCommissionEmployee};
